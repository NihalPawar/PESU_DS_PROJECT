...
char a[67] = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890.,? ";
int i=0;
for(i=0;i<67;i++)
{
insert(&first,a[i]);
...
printf("Total number of bits used to store the string = ");
printf("%d  \n\n",freq_count*2);  //display the number of bits used to represent the string.
...
if(root->left==NULL && root->right==NULL)
	{
		printf("%c : ", root->data);
		int i;
		/*FILE *file = fopen("output2.txt", "wb");
		if (file == NULL)
...
int a[index];
fp = fopen("output1.bin", "rb");
for(k=0;k<index;k++)
{
	fread(&a[k], sizeof(a[k]), 1, fp);
; 
}
    fclose(fp);
